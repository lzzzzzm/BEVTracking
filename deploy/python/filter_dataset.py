...
def main():
    ...
    train_images_path = os.path.join('dataset', 'standford_campus', 'train')
    train_anno_path = os.path.join('dataset', 'standford_campus', 'annotations', 'train.json')

    coco = COCO(train_anno_path)
    ids = list(sorted(coco.imgs.keys()))

    # create a folder to save intermediate images
    Path('dataset/standford_campus/train_bbox').mkdir(exist_ok=True)
    ...
    for img_id in ids:
        ...
        img_path = os.path.join(train_images_path, path)
        img = cv2.imread(img_path)
        for target in targets:
            index = index+1
            bbox = target['bbox']
            x,y,w,h = bbox[0], bbox[1], bbox[2], bbox[3]
            img_bbox = img[y:y+h, x:x+w]
            # resize to 256x256
            # img_bbox = cv2.resize(img_bbox, (256, 256))
            # save img_bbox to the intermediate images folder
            cv2.imwrite(f'dataset/standford_campus/train_bbox/{img_id}_{index}.jpg', img_bbox)
        break

    # from the intermediate folder load the image_list
    img_list = get_test_images('dataset/standford_campus/train_bbox', None)
